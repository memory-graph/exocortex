...
- **Context (15):** `ConcerningApplication`, `Contains`, `DependsOn`, `ForApplicant`, `Governs`, `InProject`, `InSession`, `Modifies`, `PartOf`, `PartOfApplication`, `Requires`, `Summarizes` †, `UnderRule`, `Uses`, `WrittenIn`
...
not. 61 kinds are declared; **58 are yours to use.** Asserting one
