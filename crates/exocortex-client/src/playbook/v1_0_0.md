...
- **Context (9):** `Contains`, `DependsOn`, `InProject`, `InSession`, `Modifies`, `PartOf`, `Requires`, `Uses`, `WrittenIn`
- **Integration (6):** `Bridges`, `Consumes`, `Exposes`, `IntegratesWith`, `Produces`, `Wraps`
- **Learning (6):** `BuildsOn`, `Confirms`, `Contradicts`, `Demonstrates`, `Specializes`, `Teaches`
- **Quality (5):** `Documents`, `Measures`, `Tests`, `Validates`, `Verifies`
- **Similarity (4):** `AnalogousTo`, `DifferentFrom`, `RelatedTo`, `SimilarTo` †
- **Solution (5):** `Addresses`, `AlternativeTo`, `Improves`, `Replaces`, `Solves`
- **Workflow (6):** `Automates`, `Configures`, `Creates`, `Executes`, `ParallelTo`, `Precedes`

† Computed-only (R-T14): the consolidation cycle asserts it, producers may
not. 48 kinds are declared; **47 are yours to use.** Asserting one
...
All 15 `RejectCode` variants appear above. If you meet one that doesn't,
