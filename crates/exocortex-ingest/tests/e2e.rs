...
async fn fifty_row_batch_accepted_lsn_monotonic() {
    ...
    // Producer-visible acceptance counts submitted rows, not the structural
    // grouping rows generated by the backend.
...
async fn inverse_materialized_on_write() {
    ...
    assert_eq!(ack.accepted, 3, "2 memories + 1 authored edge");
    let rels: Vec<_> = {
        let mut out = vec![];
    ...
    assert_eq!(
        ...
        exocortex_kernel::Provenance::Asserted {
            author: "test-adapter".into(),
            producer_kind: Some(exocortex_kernel::ProducerKind::CodingAgent),
        },
        "authored relationships retain authenticated producer provenance"
    );
    ...
    assert_eq!(ack2.accepted, 3, "generated rows do not inflate the ack");
