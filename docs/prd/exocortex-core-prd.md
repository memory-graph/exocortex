...
4. MERGE / ABSTRACT / PRUNE / STRENGTHEN via Storage. Merge candidates are informed by `MCR2Engine::identify_merge_candidates`. **ABSTRACT shape (D8, decided 2026-09-03):** a class with >= 3 surviving members gets ONE `Abstraction` row — carried by the `General` memory type (a new dev-v1 type would renumber every later pack's type ids in composed ontologies; distinguishability is provenance, the D23 precedent), `Computed { producer: Abstraction, threshold: mean-pairwise-cosine }` provenance, a deterministic id derived from the sorted member ids (same set ⇒ same row), narrowest-member visibility, the member-centroid embedding under the members' common model, and bounded deterministic content (member titles — structure only, no narration per R-Dr8). Membership rides the computed-only `Summarizes` kind (bucket Context, symmetric): producers cannot assert it (R-T14), Dreams and only Dreams writes it. The row and its edges commit in one fenced journaled batch, rollback-able with the cycle. `ConsolidationResult.abstracted` carries the abstraction ROW ids. Recorded boundary: membership drift across cycles derives a NEW row; retiring stale abstractions is PRUNE's discipline.
5. Create SimilarTo edges @ threshold 0.85 with `Provenance::Computed { producer: SimilarityHnsw, threshold: 0.85 }`. Ingest-time cosine seeding (`SimilarityCosine`) ~~is deferred: writing SimilarTo on the hot path would put embedding compute on the write critical path, which violates R-Lat3. All SimilarTo edges land in Dreams.~~ **shipped (D5) as an OPT-IN server flag, default off** (the recorded lean stands by default): embeddings are already computed synchronously at ingest, so the incremental cost is bounded in-memory cosine compares against the 512-row recent ring. Seeding writes the SAME window Dreams writes in — [0.85, 0.92), so near-duplicates still consolidate through the merge path — with `Computed { SimilarityCosine, 0.85 }` provenance, deterministic ids, and narrower-endpoint visibility; producers still cannot submit SimilarTo (R-T14). Best-effort: a crash between commit and seeding loses skeleton edges the next Dreams cycle writes anyway.
...
5. **Embedding model swap procedure.** ~~Dual-write? Blue/green corpora? Not yet specified. Given the model-version-stamped MCR² and cross-model prohibition (R-Mcr1), the answer is probably blue/green corpora with an explicit reindex operation.~~ **Closed (D4): blue/green with an explicit reindex.** `reindex_embeddings` (admin-gated, audited per chunk, MCP+HTTP parity) re-embeds every stored row under the configured model and restamps `EmbeddingModel`; a swap is: configure the new model → reindex → the graph is single-revision again and MCR² computes. Dual-write during transition is explicitly NOT shipped — R-Mcr1 makes a mixed graph unconsolidatable, so the honest procedure is cutover-by-reindex (idempotent, chunk-committed, restartable). Rows that committed without vectors are backfilled by the same operation.
...
12. **Ingest-time SimilarTo seeding.** §12.1 defers all `SimilarTo` writes to Dreams (`SimilarityHnsw`) to keep the write path off the embedding runtime. ~~If ingest embedding cost is measured well below the write budget, is it worth writing coarse `SimilarityCosine` edges at ingest to give the graph a warm SimilarTo skeleton before the first Dreams cycle? Lean: no in v1; the cold-start window is short once event-driven Dreams (§12.2) starts firing on real load.~~ **Closed (D5): shipped opt-in, default off.** The lean became the default; the premise resolved — embeddings were already computed synchronously at ingest (§7.5), so seeding rides the existing vectors with bounded ring compares. Dreams remains the only producer at default configuration.

13. **Org-tuned embeddings via CgMCR² (v2 north-star).** The v1 design uses a pluggable frozen embedding model. The obvious lever for making the memory system meaningfully better than a generic embedding provides is to fine-tune a small embedding model on the org's own graph, using CgMCR²'s joint clustering + rate-reduction objective. This crosses the "no training in the backend" invariant, requires deciding on a fine-tuning corpus (single-org? cross-org with visibility guarantees?), and requires a model-version-stamping protocol across fine-tunes (extending R-Mcr1). Scope for v2. Not a footnote — this is the direction the graph-as-durable-artifact premise ultimately points at. *(D4 substrate note, 2026-09-03: the deterministic half shipped — q5's explicit reindex operation, per-row model stamping, and the D22 rights-gated temporally-clean corpus export are the adoption path for any externally fine-tuned model. The training loop itself stays out-of-tree per the D23 external-producer precedent; no invariant moves.)*
