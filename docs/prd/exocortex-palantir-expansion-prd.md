...
**S7 — Change-feed seam (`ChangeLog`). NEW.** The change feed is the one major subsystem with no named contract: appending a change at an LSN lives in `cluster/src/node.rs`, mounting and authorizing the feed in `server/src/sse.rs`, and consuming with gap detection in `client/src/sync.rs`. Three crates, three partial ideas of one contract — which is how the audit's CS1 happened: an empty replay ring answered *"you are current"* instead of 409 Resync, silently dropping every invalidation between the consumer's `since_lsn` and the node's LSN. S7 names the contract as a `ChangeLog` port with four operations (append at an LSN, read from an offset, refuse an offset that has aged out, subscribe) and asserts it against the in-process implementation. **No broker.** The bounded in-memory ring stays the only implementation; the point is a written contract and a conformance suite, not new infrastructure.
